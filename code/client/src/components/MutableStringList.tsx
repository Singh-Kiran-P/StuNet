...
export default function MutuableStringList(props: Props) {
    ...
    return(
        <View>
            <TextInput mode='outlined' label={props.title} onChangeText={setElement} />
            {elements.map((item, i) => {
                return <Text key={i}>{item}</Text>})
                }
            <Button mode='contained' onPress={add} disabled={ element === ''}>Add</Button>
            <Button mode='contained' onPress={remove}>Remove</Button>
        </View>
